...
async function sessdl() {
  try {
    ...
    const base64Data = sess.SESSION_ID.split("hasi-md~")[1];
    if (!base64Data) {
...
async function connectToWA() {
  ...
  //===========================
  console.log("🔥 HASI-MD Bot is starting...");
  ...
  conn.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      let up = `
          🚀 **© Hasi-md 💚 Connected Successfully!** ✅ 
      ...
      conn.sendMessage(conn.user.id, {
        text: up,
        contextInfo: {
          mentionedJid: ["94769421870@s.whatsapp.net"], // specify mentioned JID(s) if any
          groupMentions: [],
          forwardingScore: 999,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
            newsletterJid: "120363409736868943@newsletter",
            newsletterName: "hasi-md 💚",
            serverMessageId: 999,
          },
          externalAdReply: {
            title: "© Hasi-md 𝗑ᴾᴿᴼ 💚",
...
if (!app) return;
app.get("/", (req, res) => {
  res.send("hey I am alive, Hasi-md bot Is started✅");
});
app.listen(port, () =>
